...
class Solution
{
    public static void main(String args[]) throws Exception
    {
        ...
        for (int T = 1; T < 11; T++) {
            ...
            // 후위 표기식
            char[] last_arr = last(char_arr).toCharArray();
            // 2. 후기 표기식 -> 전위로 바꾸며 계산
            int result = caluator(last_arr);
            ...
            if (operation[i] == '+' || operation[i] == '-' || operation[i] == '*' || operation[i] == '/'
                ...
                if (operation[i] == '+' || operation[i] == '-') {
                    ...
                    if (!op.empty()) {
                        ...
                        else if (op.peek() == '+' || op.peek() == '-' || op.peek() == '(') {
                            op.push(operation[i]);
                        }
                    }
                    // empty면 그냥 넣어
    ...
    public static int caluator(char[] last_arr) {
        // 이번에는 숫자를 스택에 넣어줄 것임
